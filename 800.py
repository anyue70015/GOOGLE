...
# ================= UI =================
st.set_page_config(page_title="5min 扫描器 - 修复版", layout="wide")
...
def perform_scan():
    ...
    for symbol in SYMBOLS:
        ...
        if df is None or len(df) < 100:
            log_container.write(f"{symbol}: 数据不足或失败")
            continue
        ...
        if all_green:
            triggered.append((symbol, close.iloc[-1]))

    if triggered:
        for symbol, price in triggered:
            msg = f"【5min 全绿】 {symbol} 价格 {price:.4f}"
            log_container.success(f"触发: {symbol}")
            if bot:
