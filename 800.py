...
st.title("标普500 + 纳斯达克100 扫描工具（实时显示 + 10秒防限流）")
...
if st.session_state.current_index < len(tickers):
    if st.button("开始/继续扫描（自动跑完剩余，每只10秒）", type="primary"):
        with st.spinner("扫描中（中断刷新后继续）..."):
...
st.caption("10秒完整版：点一次按钮自动跑完剩余，断点续扫 + 实时排序 + 结果不丢！")
