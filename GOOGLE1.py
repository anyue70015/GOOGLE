...
st.set_page_config(page_title="极品短线-三高筛选版", layout="wide")
st.title("🎯 极品短线扫描 (得分≥3 + 胜率≥70% + PF7≥3.5)")
...
def compute_metrics(symbol, cfg_key):
    ...
    # 计算近三天变化
    if len(c) >= 4:
        chg_3d = [(c[-1]/c[-2]-1)*100, (c[-2]/c[-3]-1)*100, (c[-3]/c[-4]-1)*100]
    else:
        chg_3d = [0.0, 0.0, 0.0]
    ...
    return {"symbol": symbol, "price": c[-1], "score": score, "prob7": prob7, "pf7": pf7, "chg_3d": chg_3d}

# ==================== 侧边栏 & 全量逻辑 ====================
st.sidebar.header("🔍 单股深度穿透")
single_sym = st.sidebar.text_input("代码 (如 SNDK/WDC)", "").upper()
if single_sym:
    for p in ["3个月", "1年"]:
        m = compute_metrics(single_sym, p)
        if m: st.sidebar.write(f"**{p}**: 得分{m['score']} | 胜率{m['prob7']*100:.1f}% | PF:{m['pf7']:.2f}")

use_strict_filter = st.checkbox("🚀 开启超级精选 (得分≥3 & 胜率≥70% & PF7≥3.5)", value=False)
mode = st.selectbox("扫描回测周期", list(BACKTEST_CONFIG.keys()), index=1)
...
if st.session_state.high_prob:
    ...
    df_sorted = df.sort_values(by=['score', 'prob7', 'pf7'], ascending=[False, False, False])
    
    if use_strict_filter:
        df_prime = df_sorted[(df_sorted['score'] >= 3) & (df_sorted['prob7'] >= 0.70) & (df_sorted['pf7'] >= 3.5)].copy()
        st.subheader(f"💎 超级精选 (共 {len(df_prime)} 只)")
    else:
        df_prime = df_sorted[(df_sorted['score'] >= 3) | (df_sorted['prob7'] >= 0.68)].copy()
        st.subheader(f"🔥 全量精选 (共 {len(df_prime)} 只)")

    for _, row in df_prime.iterrows():
        c3 = row['chg_3d']
        chg_html = f"<span style='color:{'#ff4b4b' if c3[0]>0 else '#00ff41'}'>{c3[0]:+.2f}%</span>, " \
                   f"<span style='color:{'#ff4b4b' if c3[1]>0 else '#00ff41'}'>{c3[1]:+.2f}%</span>, " \
                   f"<span style='color:{'#ff4b4b' if c3[2]>0 else '#00ff41'}'>{c3[2]:+.2f}%</span>"
        
        st.markdown(
            f"""<div style="border-left: 6px solid #00FF00; padding: 12px; margin: 10px 0; background-color: #f0f2f611;">
                <b style="font-size:18px;">{row['symbol']}</b> | 价格: ${row['price']:.2f} | 
                <b>得分: {row['score']}/5</b> | 胜率: {row['prob7']*100:.1f}% | PF7: {row['pf7']:.2f}<br>
                <small>近三天涨跌: {chg_html} (左为最新)</small>
            </div>""", unsafe_allow_html=True
        )

    # 导出报告
    report_lines = [f"报告生成: {time.strftime('%Y-%m-%d %H:%M')}", "-"*30]
    for _, row in df_prime.iterrows():
        report_lines.append(f"{row['symbol']}: 得分{row['score']} | 胜率{row['prob7']*100:.1f}% | PF7:{row['pf7']:.2f} | 3日 chg:{row['chg_3d']}")
    st.download_button("📥 导出精选报告", "\n".join(report_lines).encode('utf-8'), "Prime_Report.txt")
