...
def get_top_300_stocks():
    ...
    try:
        ...
        st.success(f"获取成功: {len(stock_dict)} 只股票")
        return stock_dict
        
    except Exception as e:
        st.warning(f"获取失败，使用备用股票池: {str(e)[:100]}")
        return get_backup_stocks()
...
def get_backup_stocks():
    ...
    cyb_samples = {
        ...
        "300750": "宁德时代", "300413": "芒果超媒"
    }
    
    backup_stocks.update(kcb_samples)
    backup_stocks.update(cyb_samples)
...
def fetch_yf_data(stock_code, days=180):
    ...
    try:
        ...
        if len(volumes) >= 20:
            ...
            turnover = avg_volume * current_price / 1e8  # 转换为亿元
        else:
        ...
        return {
            ...
            'turnover': round(turnover, 2)  # 成交额（亿元）
        }, True, "成功"
...
def calculate_signals(stock_data):
    ...
    # MACD信号
    if macd_hist[-1] > 0 and (len(macd_hist) < 2 or macd_hist[-2] <= 0):
    ...
    return {
        'score': score,
        'signals': signals,
        'win_rate': max(0, min(100, round(base_win_rate, 1))),
        'profit_factor': max(0, round(base_profit_factor, 2)),
        'rsi': round(rsi[-1], 1),
...
with st.sidebar:
    ...
    delay_time = st.slider("延迟(秒)", 0.1, 3.0, 0.8, 0.1)
    
    st.markdown("---")
    st.info(f"股票总数: {len(STOCK_POOL)}")
    st.info(f"科创板: {len([c for c in STOCK_POOL if c.startswith('688')])}")
    st.info(f"创业板: {len([c for c in STOCK_POOL if c.startswith('300')])}")
    ...
    if scanned_count < total_stocks:
        ...
        for i in range(scanned_count, batch_end):
            ...
            if result['评级'] == '🔥 优质':
                ...
                st.success(f"🎯 {stock_code} {stock_name} | "
                          f"价:{result['价格']} | 涨:{result['涨幅%']:+.2f}% | "
                          f"分:{result['信号分']} | 胜:{result['胜率%']}% | "
                          f"PF:{result['盈亏比']:.2f}")
        ...
        st.session_state.scanning = False
        st.balloons()
        st.success(f"✅ 完成! 扫描{total_stocks}只, 优质{st.session_state.premium_count}只")
    ...
    if not df_valid.empty:
        ...
        if not premium_stocks.empty:
            ...
            for idx, (_, stock) in enumerate(premium_stocks.iterrows(), 1):
                ...
                txt_content += f"价:{stock['价格']:7.2f} 涨:{stock['涨幅%']:+6.2f}% "
                txt_content += f"分:{stock['信号分']:1d} RSI:{stock.get('RSI', 0):5.1f} "
                txt_content += f"量:{stock.get('量比', 0):4.1f}x "
                txt_content += f"胜:{stock['胜率%']:5.1f}% "
                txt_content += f"PF:{stock['盈亏比']:5.2f} "
                txt_content += f"额:{turnover:5.2f}亿\n"
                txt_content += f"    信号: {stock['触发信号']}\n"
                txt_content += "-" * 80 + "\n"
            
            # 显示和下载TXT
            st.text_area("优质股票详情", txt_content, height=250)
        ...
        # 显示完整结果
        st.subheader(f"📋 完整结果 ({len(df_valid)}只)")
        ...
        for _, stock in df_valid.iterrows():
            ...
            full_txt_content += f"分:{stock['信号分']:1d} RSI:{rsi_value:5.1f} "
            full_txt_content += f"量:{volume_ratio:4.1f}x "
            full_txt_content += f"胜:{stock['胜率%']:5.1f}% "
            full_txt_content += f"PF:{stock['盈亏比']:5.2f} "
            full_txt_content += f"额:{turnover:5.2f}亿\n"
        ...
        full_txt_content += f"- 平均胜率: {df_valid['胜率%'].mean():.2f}%\n"
        full_txt_content += f"- 平均盈亏比: {avg_pf:.2f}\n"
        full_txt_content += f"- 优质股票比例: {len(premium_stocks)/len(df_valid)*100:.1f}%\n"
        full_txt_content += "=" * 100
        
        # 显示和下载完整TXT
        with st.expander("📄 查看完整TXT结果"):
            st.text_area("完整结果", full_txt_content, height=350)
